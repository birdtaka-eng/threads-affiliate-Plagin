...
function onOpen() {
  ...
  ui.createMenu('💎 Sanctuary')
    ...
    .addSeparator()
    .addItem('🔥【2-A】魂の排出（人格A・偏愛）', 'generateTripleSetsA')
    .addItem('🌟【2-B】魂の排出（人格B・勝利）', 'generateTripleSetsB')
    .addSeparator()
    .addItem('🔍【3】モデル名を確認', 'listAvailableGeminiModels')
    .addSeparator()
    .addItem('🆔【ID】Threads IDを自動取得', 'fetchThreadsUserId')
    .addItem('🛡️【Check】接続テスト', 'verifyAllSettings')
    .addSeparator()
